...
> **사람이 읽는 버전**: 학생 등 Claude 없이 이 저장소를 쓰는 사람을 위한 문제 해결 가이드가
> `docs/TROUBLESHOOTING.md`에 있다 (이 스킬의 내용을 비개발자도 따라할 수 있게 정리한 것).
> **이 스킬에 새 문제를 추가하면 그 문서에도 함께 반영할 것.**
...
### 최종 확인
사용자가 Firestore 데이터베이스를 생성하고 규칙을 적용한 뒤, **버튼 저장·새로고침 후 유지·재생까지 모두 정상 동작하는 것을 확인했다.** 이것으로 문제 1~10 전부 해결 완료 (2026-07-09).
