...
// JS가 "끝났다"는 신호를 영영 읽지 못해 화면이 100%에서 멈춘 것처럼 보이는 문제가 있었다
...
// 네트워크 문제로 요청이 끝나지도, 에러를 던지지도 않고 그냥 멈춰버리는 경우를 대비한 안전장치.
...
function withTimeout<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timeoutId = setTimeout(() => {
      reject(
        new Error(
          `업로드가 ${timeoutMs / 1000}초 안에 끝나지 않았습니다. 네트워크 상태를 확인하고 다시 시도해주세요.`
        )
      );
    }, timeoutMs);
...
export async function uploadAudioFile(
  ...
  onProgress?.(0);
  await withTimeout(uploadBytes(storageRef, blob), UPLOAD_TIMEOUT_MS);
  onProgress?.(1);
